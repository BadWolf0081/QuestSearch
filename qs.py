...
async def qform(ctx, areaname="", pokemon_name="", form_query=""):
    ...
    try:
        ...
        # Fuzzy match Pokémon
        mon = fuzzy_find_pokemon(pokemon_name)
        if not mon:
            await ctx.send(f"Could not find Pokémon: {pokemon_name}")
            return

        # Fuzzy match form for this Pokémon
        form_id, form_name = lookup_form_id_for_mon(mon_id=int(mon['pokedex'].split('(')[-1].replace(')', '').strip()), form_query=form_query)
        if not form_name:
        ...
        quests = await get_data(area, int(mon['pokedex'].split('(')[-1].replace(')', '').strip()))
        found = False
